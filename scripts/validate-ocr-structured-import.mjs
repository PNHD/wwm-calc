...
const assertExpected = (label, parsed) => {
  ...
  expect(rows.filter((sub) => sub.isTuned).length === 1 && rows.find((sub) => sub.isTuned)?.type === "Max Physical Attack", `${label}: only [Turn] Max Physical Attack may be Retuned`);
...
expect(flatRows.find((row) => row.type === "Max Physical Attack")?.val === "73", "Max Physical Attack 73 must remain 73");
