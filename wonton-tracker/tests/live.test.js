...
import { createLiveState, observedStats, recordActualReforge, recordActualResult, undoLive } from '../core/live.js';
...
test('Gold resets only selected slot and records interval', () => {
  const state = createLiveState();
  state.slots[1].active = true;
  state.slots[1].progress = 100;
  ...
  const result = recordActualResult(recorded, { goldSlots: [2] }).state;
  assert.equal(result.slots[0].pity, 21);
  assert.equal(result.slots[1].pity, 0);
  assert.deepEqual(result.observedGoldIntervals[2], [41]);
});

test('sequential unlock starts new pity at zero and Slot 5 is fixed Gold', () => {
  let state = createLiveState();
  for (let target = 2; target <= 5; target += 1) {
    for (let attempt = 0; attempt < 30; attempt += 1) {
      state = recordActualReforge(state).state;
      state = recordActualResult(state).state;
    }
...
test('an observed early unlock activates only the next slot', () => {
  let state = recordActualReforge(createLiveState()).state;
  state = recordActualResult(state, { unlockEarly: true }).state;
...
test('a full-progress activation cannot also early-unlock the following slot on the same reforge', () => {
  ...
  const result = recordActualResult(recorded, { unlockEarly: true }).state;
