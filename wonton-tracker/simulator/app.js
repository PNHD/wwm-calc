(function () {
    ...
    els.history.innerHTML = state.history.slice(0, 30).map(event => {
      ...
      return `
        <div class="history-item">
          <div class="history-head"><strong>Roll #${event.roll}</strong><span>${event.cost} stone${event.cost === 1 ? '' : 's'} · ${beadsForStones(event.cost).toLocaleString()} beads · ${formatUsd(event.cost)}</span></div>
          <div>${changes}</div>
        </div>`;
    }).join('');
  }
  ...
  function renderMetrics() {
    ...
    els.totalValue.textContent = `${beadsForStones(state.totalStones).toLocaleString()} Echo Beads · ≈ ${formatUsd(state.totalStones).replace('~
    ...
    els.nextValue.textContent = `${beadsForStones(nextStones).toLocaleString()} Echo Beads · ${formatUsd(nextStones)}`;
  ...
  function runBatch() {
    ...
    els.batchResult.innerHTML = `
      <div class="batch-grid">
        ...
        <div><span>Avg. cost</span><strong>${result.averageStones} stones</strong><small>${Math.round(beadsForStones(result.averageStones)).toLocaleString()} beads · ${formatUsd(result.averageStones)}</small></div>
        <div><span>Median cost</span><strong>${result.medianStones} stones</strong><small>${beadsForStones(result.medianStones).toLocaleString()} beads · ${formatUsd(result.medianStones)}</small></div>
        <div><span>P90 cost</span><strong>${result.p90Stones} stones</strong><small>${beadsForStones(result.p90Stones).toLocaleString()} beads · ${formatUsd(result.p90Stones)}</small></div>
      </div>
