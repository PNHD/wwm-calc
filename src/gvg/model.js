import { cloneBoundedJson, inspectBoundedJson, isPlainRecord } from "../product/storage-registry.js";

// V1_STORAGE_HARDENING_GVG
// COMPETITIVE_V2_GVG_ATTUNEMENT_SANITIZER_UNKNOWN — generated sanitizeGvgWorkspaceV1 is patched by apply-competitive-v2-pre-hardening.mjs and verified by validate-gvg-model.mjs.
...
export function rosterSignature(roster = []) {
  return [...roster]
    .map((member) => `${member.id}:${member.path}:${(member.roles ?? []).join(",")}:${member.exLevel ?? 1}:${member.gvgSelectedProfile ?? "UNKNOWN"}:${member.availability !== false}`)
    .sort()
    .join("|");
}
...
export function defaultWorkspace() {
  return {
    ...
    matchLogs: [],
    attunementProfiles: { normal: { name: "PvE / Normal", source: "legacy-compatible" }, arena: { name: "Arena", source: "separate-profile" }, gvgSelected: "UNKNOWN" /* COMPETITIVE_V2_GVG_ATTUNEMENT_UNKNOWN_DEFAULT */ },
  };
}
