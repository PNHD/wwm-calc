...
export default function GearCompareWorkspace({ rows, slots, activeSlot, onSlotChange, onEquip, onEdit }: Props) {
  ...
  return <main className="compare-workspace" id="main-content">
    ...
    <section className="compare-grid" aria-label="Gear comparison candidates">{candidates.map((row) => {
      ...
      return <article key={row.id} className={row.equipped ? "is-current" : ""}><header><img src={row.image} alt="" /><div><strong>{row.name}</strong><small>{row.slotLabel} / {row.setName}</small></div><b>{Math.round(modeledDps).toLocaleString()} DPS</b></header><div className="compare-lines">{row.subs.slice(0, 6).map((sub, index) => <span key={`${sub.type}-${index}`}><strong>{sub.type}{sub.tuned ? " (tuned)" : ""}</strong><b>{sub.value}</b></span>)}</div><footer><span>{row.equipped ? <><Check size={14} aria-hidden="true" /> Current complete build</> : <>{deltaDps >= 0 ? "+" : ""}{Math.round(deltaDps).toLocaleString()} DPS ({deltaPct >= 0 ? "+" : ""}{deltaPct.toFixed(2)}%)</>}</span><button type="button" onClick={() => onEdit(row.id)} aria-label={`Edit ${row.name}`}><Pencil size={15} /></button>{!row.equipped && <button type="button" className="is-primary" onClick={() => onEquip(row.id)}>Swap gear</button>}</footer></article>;
    })}</section>
  </main>;
}
