...
import { getCapabilityState } from "../../data/pathCatalog";
...
const candidateDps = (row: GearCompareRow): number | null => row.available === false ? null : row.modeledDps ?? row.score ?? null;
...
export default function GearCompareWorkspace({ rows, slots, activeSlot, pathKey, onSlotChange, onEquip, onEdit }: Props) {
  ...
  const provisional = getCapabilityState(pathKey, "gearCompare") === "PROVISIONAL";
  const candidates = rows
    .filter((row) => (activeSlot === "ALL" || row.slot === activeSlot) && `${row.name} ${row.setName}`.toLowerCase().includes(query.toLowerCase()))
    .sort((a, b) => (candidateDps(b) ?? -Infinity) - (candidateDps(a) ?? -Infinity));
  ...
  return <main className="compare-workspace compare-workspace-v2" id="main-content">
    ...
    <section className="compare-grid compare-grid-v2" aria-label="Gear comparison candidates">
      {candidates.map((row, index) => {
        ...
        const isRecommended = provisional && row.available !== false && !row.equipped && deltaDps > 0 && index === candidates.findIndex((candidate) => candidate.available !== false && !candidate.equipped && candidateDeltaDps(candidate) > 0);
        const resultLabel = row.available === false ? "UNAVAILABLE" : row.equipped ? "CURRENT" : deltaDps > 0 ? (isRecommended ? "MODEL LEAD" : "MODEL HIGHER") : deltaDps < 0 ? "MODEL LOWER" : "MODEL TIE";
        ...
        return <article key={row.id} className={`${row.equipped ? "is-current" : ""} ${isRecommended ? "is-recommended" : ""}`}>
          ...
          {row.available === false ? <section className="compare-primary-result" aria-label={`${row.name} result`}>
            <div><small>MODELED DPS</small><strong>N/A</strong></div>
            <div><span><small>VS CURRENT</small><strong>Unavailable</strong></span></div>
            <span className="compare-confidence is-experimental">SCENARIO CONTRACT</span>
          </section> : <section className="compare-primary-result" aria-label={`${row.name} result`}>
            <div><small>PROVISIONAL MODEL DPS</small><strong>{Math.round(modeledDps!).toLocaleString()}</strong></div>
          ...
          {row.available === false && <section className="compare-why"><h3>Comparison unavailable</h3><p>{row.unavailableReason}</p></section>}

          {row.available !== false && !row.equipped && <section className="compare-why">
